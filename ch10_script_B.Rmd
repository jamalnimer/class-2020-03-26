...
The unit is a single person that is surveyed as well as all the variables that are attached to them. 
The treatment is whether or not there was a Spanish speaker near them. The outcome is the attitude_change. 
The causal effect is whether or not the Spanish speaker affected the attitude change. 
The fundamental problem of inference is that we cannot assign both the treatment and control to the same person so we don't know for *certain* whether or not the causal effect is true.
...
ggplot(train, aes(x = treatment, y = att_chg)) +
  ...
  geom_point(aes(x = 1, y = avg_control), color = 'blue') +
  geom_point(aes(x = 2, y = avg_treatment), color = 'red') +
  xlab('Treatment') +
  ylab('Attitude Change')

# there is an attitude change because 
# people won't always express the same attitude.
```
...
train <- train %>% 
  mutate(treatment_numeric = as.numeric(treatment) - 1) 

train %>% 
  select(treatment, treatment_numeric)

ggplot(train, aes(x = treatment_numeric, y = att_chg)) +
  geom_jitter(width = 0.2) +
  geom_point(aes(x = 0, y = avg_control), color = 'blue') +
  geom_point(aes(x = 1, y = avg_treatment), color = 'red') +
  xlab('Treatment') +
  ylab('Attitude Change') +
